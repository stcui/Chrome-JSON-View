...
function load() {
	var child, data;
	var firstChar = document.body.textContent.charAt(0);
	if (document.body && (document.body.childNodes[0] && document.body.childNodes[0].tagName == "PRE" || document.body.childNodes.length == 0 || firstChar == '[' || firstChar == '{')) {
		child = document.body.children.length ? document.body.childNodes[0] : document.body;
		data = extractData(document.body.textContent);
		if (data)
			init(data);
	}
}
